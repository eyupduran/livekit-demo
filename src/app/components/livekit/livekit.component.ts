...
export class LivekitComponent implements OnInit {
  ...
  startTime: number;
  @ViewChild('participantsArea', { static: false }) participantsArea: ElementRef;
...
-
    this.room
      ...
      .on(RoomEvent.LocalTrackPublished, (pub) => {
        ...
        if (track instanceof LocalAudioTrack) {
          ...
          setInterval(() => {
            const codecElm = <HTMLElement>document.getElementById("local-volume-"+identity)
  ...
  renderParticipant(participant: Participant, remove: boolean = false) {
    if (!this.participantsArea) return;
    const { identity } = participant;
    let div = <HTMLElement>document.getElementById("participant-" + identity);
    // let div = this.participantsArea.nativeElement.querySelector(
    ...
    if (!div && !remove) {
      ...
      this.renderer.appendChild(this.participantsArea.nativeElement, div);
      let sizeElm = <HTMLSpanElement>document.getElementById("size-" + identity);
      //let sizeElm = $(`size-${identity}`) as HTMLElement;
    ...
    if (remove) {
      div.remove();
      this.renderer.removeChild(this.participantsArea.nativeElement.parentNode, this.participantsArea.nativeElement);
      let el =<HTMLElement>document.getElementById('participants-area')
  ...
  handleRoomDisconnect(reason?: DisconnectReason) {
    ...
    // const container = this.participantsArea.nativeElement
     const container = this.participantsArea.nativeElement;

    if (container) {
      this.renderer.removeChild(container.parentNode, container.nativeElement);
    }
  ...
  renderScreenShare(room: Room) {
    ...
    // let div = this.screenshareArea.nativeElement
    ...
    console.log("SHARE SCREEN",(<HTMLElement>document.getElementById("screenshare-area")))
  }
